...
- **v2.0 特色**（已歸檔至 `archive-versions/`）：
  ...
  - ✅ 完整的 DEBUG 模式
- **v3.0 新增特色**（2026-01-24）：
  - ✅ Day 2 動態料理系統（依收集記憶顯示可做料理：蜜汁燉菜、苦辛醒神湯、撫慰鹹粥）
  - ✅ 料理記錄系統（dishesCooked）用於 Day 3 結局額外台詞
  - ✅ 配方檢查機制（防止食材不足時做料理）
  - ✅ 多料理記憶劇場（苦辛醒神湯：最後一針+雪中；撫慰鹹粥：雪子靜靜陪伴）
  - ✅ Day 3 結局動態台詞（依做過的料理追加「雪中的事...」「謝謝你陪著我...」）
...
6. 修改以下配置（在檔案開頭的配置區）：
   ```javascript
   const SPREADSHEET_ID = '你的_Google_Sheets_ID';
   const CONFIG = {
