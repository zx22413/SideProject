...
> **當前版本**：V4.10（Day 1-2 料理演出動態化）  
> **最後更新**：2026-01-30  
> **狀態**：✅ 完整動態料理系統 + Script Properties 安全配置 + clasp 跨電腦同步
...
- **版本**：V4.10（2026-01-30）
- **行數**：~6,174 行
- **狀態**：✅ 完整動態料理系統 + Script Properties 安全配置

**V4.10 新增特色**（2026-01-30）：
- ✅ Day 1-2 料理演出動態化（有啥食材顯示啥）
- ✅ Day 1 熱茶/熱湯根據實際收集的記憶動態顯示
- ✅ Day 2 蜜汁燉菜移除錯誤的「寒冷」
- ✅ Day 2 苦辛醒神湯記憶劇場改為 Flex Card
- ✅ Script Properties 安全配置（敏感資訊不再寫死在程式碼）
- ✅ clasp 跨電腦同步支援

**V4.9 新增特色**（2026-01-29）：
- ✅ Day 3 最終料理動態化（糖霜幻景拼盤/千針冷骨湯/百味蜜汁炙燒魚）
- ✅ 修正苦味結局真相獨白：「職人的徒勞」主題
- ✅ Day 3 料理演出動態化（Part1/Part2/MemoryCard）
...
## 🔄 clasp 跨電腦同步（V4.10 新增）

使用 Google 官方的 `clasp` CLI 工具，可以在本機編輯程式碼，然後一鍵同步到 GAS。

### 為什麼要用 clasp？

| 傳統方式 | clasp 方式 |
|---------|-----------|
| 複製貼上到 GAS 網頁 | 終端機打 `clasp push` |
| 手動從 GAS 下載 | 終端機打 `clasp pull` |
| 換電腦要重新複製 | Git + clasp 自動同步 |
...
1. 打開：https://script.google.com/home/usersettings
2. 找到「Google Apps Script API」→ 打開（ON）
3. 等 1-2 分鐘讓設定生效
...
**路徑**：`D:\User\SideProject\01-開發實驗\程式碼`

```powershell
# 開始工作前（如果公司有改過）
cd "D:\User\SideProject\01-開發實驗\程式碼"
git pull
clasp pull
...
| 文件 | 用途 | Git 追蹤？ |
|-----|------|----------|
| `.clasp.json` | 連結 GAS 專案的配置 | ✅ 是 |
| `appsscript.json` | GAS 專案設定 | ✅ 是 |
| `.claspignore` | 忽略不需推送的文件 | ✅ 是 |
...
**原因**：npm 全域安裝路徑沒有加入 PATH（Windows PowerShell 常見問題）

**解決方案（三選一）**：

1. **完全關閉終端機再重開**（讓 PATH 重新載入）
...
3. **手動加入 PATH**：
   - 執行 `npm config get prefix` 查看全域路徑（通常是 `C:\Users\你的帳號\AppData\Roaming\npm`）
   - 將該路徑加入系統環境變數

#### ❌ `Project settings not found`

**原因**：在錯誤的目錄執行 clasp（`.clasp.json` 不在當前目錄）

**解決方案**：
```powershell
# 先切換到正確目錄（有 .clasp.json 的地方）
cd "D:\User\SideProject\01-開發實驗\程式碼"   # 家裡
cd "E:\SideProject\01-開發實驗\程式碼"        # 公司
...
1. **clasp 配置會跟著 Git 同步**，家裡電腦 `git pull` 後不用重新設定專案連結
2. **Script Properties 不會被 clasp 同步**，每台電腦的 GAS 後台都要設定一次（但通常只有一個 GAS 專案，所以設定一次就好）
3. **避免推送敏感資訊**，程式碼已改用 Script Properties，可以安全推送
4. **clasp pull 會產生 .js 檔案**，這是正常的，可以刪除（.gs 和 .js 內容相同）
