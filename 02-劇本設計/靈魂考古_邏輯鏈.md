# 靈魂考古邏輯鏈：誘捕・偵訊・赦免的記憶喚起機制

> **設計目標**：單輪內多階段考古，強化單位時間的情感濃度（參考 Bird Alone 敘事結構）  
> **參考原型**：[[孤鳥劇情解析與文字遊戲發想]]、《Bear's Restaurant》、《Spiritfarer》  
> **技術實現**：現以 **Ink** 為劇情 SSOT；對應 knot／flow 見 [[專案實作對照表]]。  
> **建立日期**：2026-01-15
...
### 單輪內階段轉折（多日養成 vs 單輪多階段）

| 維度 | Bird Alone（多日養成） | 靈魂食堂（單輪多階段） | 設計理由 |
|------|------------------|----------------|----------|
| **敘事結構** | 線性成長 → 衰老 → 死亡 | 記憶喚起 → 執念解開 → 昇華 | 避免「養成疲勞」，聚焦「解謎快感」 |
| **情感曲線** | 緩坡型（漸進式依戀） | 山峰型（集中爆發） | LINE 碎片化使用情境更適合短周期 |
| **重玩動力** | 低（結局重複） | 高（每位靈魂故事不同） | 單元劇結構支撐長期營運 |
| **技術成本** | 多天推播 × N用戶 | 依玩家推進、無需推播 | 零推播成本 |

**核心洞察**：
> 多日養成的精髓不在「長度」，而在「階段性的情感轉折」。  
> 將這些轉折收束為誘捕・偵訊・赦免，單次完結的弧線更適合文字遊戲體驗。
...
#### Day 1 結束條件
- 收集至少 **3 個**感官標籤
- 玩家可於同一輪內推進至下一階段（誘捕 → 偵訊 → 赦免）

**設計意圖**：階段性轉折創造「懸念」與「推進感」；當前產品為單輪可完結（見 [[../00-核心企劃/開發決策鏈]] 決策點 5）。
...
### 問題 3：一口氣玩完單輪
**解決**：
- 可選：強制隔日解鎖（檢查 `lastActive` 日期差）；目前產品為依玩家推進、不綁日曆（見 [[../00-核心企劃/開發決策鏈]]）
- 第一階段完成後，Rich Menu 鎖定「接待客人」按鈕

---

## 📋 下一步開發檢查清單

- [ ] 撰寫 Guest 1「奶奶的羊肉湯」完整劇本（3 階段 × 5個對話節點 = 15個 dialogue entries）
